...
class ColPosList extends AbstractItemsProcFunc
{
    ...
    public function itemsProcFunc(&$params)
    {
        if ($params['row']['pid'] > 0) {
            ...
            $existingElement = $GLOBALS['TYPO3_DB']->exec_SELECTgetSingleRow(
                'pid, CType',
                'tt_content',
    ...
    protected function addColPosListLayoutItems($pageId, array $items, $CType = '')
    {
        ...
        if ($layout) {
            if ($CType !== '' && count($layout['__items']) > 0) {
                foreach ($layout['__items'] as $itemKey => $itemArray) {
                    if ($itemArray[3] !== '' &&
                        !GeneralUtility::inList($itemArray[3], $CType) &&
                        !GeneralUtility::inList($itemArray[3], '*')
                    ) {
        ...
        $items[] = array(
            $GLOBALS['LANG']->sL(
                'LLL:EXT:dw_content_elements/Resources/Private/Language/locallang_db.xlf:e1301InfoTool.ColPos'
            ),
            '-2',
            null,
            null
